...
export default function NotificationsPage() {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      setFilteredNotifications(applyFilters(data.notifications, filter, searchQuery, dateFilter));
    } catch (error) {
      toast.error("Erro", {
  ...
  const applyFilters = (notifs: Notification[], filterStatus: string, query: string, dateRange: DateFilter) => {
    ...
    try {
      ...
      setNotifications(prev => 
        prev.map(notif => ({
          ...
          readAt: notif.status === "UNREAD" ? new Date() : notif.readAt
        }))
      );
        ...
        {loading ? (
          ...
          <>
            {filteredNotifications.length === 0 ? (
              ...
              <div className="space-y-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 p-4">
                {filteredNotifications.map((notification) => (
                  <div 
                    key={notification.id}
                    className={cn(
                      "border rounded-lg p-4 transition-colors bg-emerald-200/50 hover:bg-emerald-300 border-emerald-300 hover:border-emerald-300",
                      notification.status === "UNREAD" && "bg-emerald-500/50 dark:bg-blue-900/10"
                    )}
                  >
